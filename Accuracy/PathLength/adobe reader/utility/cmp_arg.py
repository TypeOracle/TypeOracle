...
def calculate_arg_type_num(self, arg_type_set):
    res_set = set()
    primitive = ['0', '1', '2', '3']
...
def compare_setter(dic1, dic2):
    ...
    content += '  - %s: %s\n' % (P1, convert_num2str(t1))
    content += '  - %s: %s\n' % (P2, convert_num2str(t2))
    if t2 == '2' or t2.startswith('5') or t2.startswith('21'): # overlook built-in object
            typeoracle[t1] -= 1
    if flag == False:
        # tmp = 'setter:%s\n- p1: %s\n- p2: %s\n' % (
...
def compare_json(dic1, dic2):
    ...
    for k in sumk:
        content += "- parameter%d:\n"%(index)
        index += 1
        if d2[k] == '2' or d2[k].startswith('5') or d2[k].startswith('21'): # overlook built-in object and value
            typeoracle[d1[k]] -= 1
        if k not in d1:
            # content += '- p1 lack key: %s\n' % (k)
            content += '  - %s: Missing\n' % (P1)
            content += '  - %s: %s\n' % (P2, convert_num2str(d2[k]))
            flag = 1
        elif k not in d2:
...
def compare_array(dic1, dic2):
    ...
    for ind in range(sumk):
        ...
        if ind >= len(l1):
            ...
            content += '  - %s: %s\n' % (P1, convert_num2str(l1[ind]))
            t1 = l1[ind]
        content += '  - %s: %s\n' % (P2, convert_num2str(l2[ind]))
        t2 = l2[ind]
        tp = compare_value(t1, t2)
        if (t2 == '2' or t2.startswith('5') or t2.startswith('21')) and t1 in typeoracle.keys(): # overlook built-in object and value
            typeoracle[t1] -= 1
        if tp == False:
            flag = 1
            if t1 in correct_report.keys():
                correct_report[t1] -= 1
